...
**CRITICAL RULE: REUSE EXISTING CLASSES.** Do not write new CSS classes or inline styles unless absolutely necessary. The design system is comprehensive. Check `app/globals.css` and existing components before adding any new utility classes or custom CSS. Rely heavily on the predefined design tokens for absolute consistency.

When building or modifying UI pages, follow these strict design conventions to ensure absolute consistency with the existing design system.

### 1. Typography

- **Headings (h1 - h5)**: Use `font-family: 'Poppins', serif;`, weight 500, with a slight negative letter spacing (`-0.015em`).
- **Body Text**: Use the `Inter` sans-serif font (`font-family: 'Inter', sans-serif;`), mostly light (weight 300) with `text-charcoal` or `text-earth` colors for maximum readability.
- **Eyebrow Text**: Use the `.eyebrow` class for section prefixes (tiny, uppercase, wide tracking). Use `.eyebrow-rule` for the animated horizontal line.
...
- **Reveal Animations**: Apply `.reveal` to individual elements. Use `.reveal-stagger` on parent containers whose children use a per-item CSS variable when sequencing is required.
- **Hero Items**: Use `.hero-item` for hero elements with the existing delayed transition behavior.
- **Breathe**: Use `.breathe` on prominent images for the existing slow floating effect.
- **Images**: Wrap images in `.img-wrap`. Use Next.js `<Image>` where appropriate; preserve the existing hover scale behavior.
- **Scroll Detection**: Reuse the shared client-side IntersectionObserver implementation to toggle `.is-in`. Do not create page-specific observers unless absolutely necessary.

### 5. Mobile & Responsive Refinements

- Use standard Tailwind breakpoints (`sm:`, `md:`, `lg:`, `xl:`).
- Compress hero-section vertical padding below `640px`.
- For the mobile menu, reuse `.hamburger` and the mobile-menu drawer/component with `.mobile-backdrop`. Keep interactive menu logic inside a client component.

### 6. Iconography

- Reuse clean, minimal SVG icons.
- Prefer the existing Tabler or Lucide icon library.
- Custom SVG icons should use `stroke-width="1.8"` and `stroke-linecap="round"`.

### 7. Custom Form UI Elements

- **Grouped Inputs**: Combine related inputs into a cohesive `.flex.bg-white.border.border-line.rounded-lg.overflow-hidden` wrapper. Apply focus states to the wrapper and keep inner inputs `bg-transparent focus:outline-none`.
- **Split Custom Inputs**: For structured values such as dates, use controlled React inputs synced to the canonical form value rather than relying on native date UI when the design requires a custom experience.
- **Radio Chips (`.chip`)**: For choices under ~5 items, use hidden native radios with Tailwind `peer-checked` states. Use `.flex-1` and `w-full justify-center` for equal-width choices where appropriate.
- **Centering Forms**: Use `.min-h-[100dvh].flex.items-center.justify-center.py-24` or equivalent spacing for large forms. Apply `m-auto` to the main `.card` when needed to prevent clipping on small screens.
- **Multi-step Progress**: Use React state for branching/multi-step flows. Reuse `.onboard-step` and `.step-dot` where those existing components/classes are part of the project.

### 8. Dashboard & Data Views

- **Grids over Tables**: For entity lists with minimal columns, prefer responsive card grids (`grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4`) over HTML tables.
- **Custom Dropdowns**: Avoid native `<select>` elements when a custom filter UI is required. Reuse `.dropdown-container`, `.dropdown-trigger`, and `.dropdown-menu`.
- **Mobile Action Menus**: Three-dot actions must remain visible on touch devices. Use `opacity-100 md:opacity-0 group-hover:opacity-100 focus:opacity-100`.
- **Header Buttons**: Page-header CTAs should stay compact on mobile. Use `self-end sm:self-auto`.
- **Table Layouts**: If a table is required, give `<th>` elements explicit percentage widths such as `w-1/2` or `w-1/4` where necessary.
